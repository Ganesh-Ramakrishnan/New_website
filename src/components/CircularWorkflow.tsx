...
const CircularWorkflow: React.FC = () => {
  ...
  const renderIcon = (iconType: string, x: number, y: number, isActive: boolean) => {
    const color = "#ffffff";
    
    switch (iconType) {
      case 'web':
        return (
          <svg x={x - 10} y={y - 10} width="20" height="20" viewBox="0 0 24 24" fill={color}>
