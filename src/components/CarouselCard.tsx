import { BarChart3, Cloud, GitBranch, Globe, Shield, Users, Zap } from 'lucide-react';
...
const cards = [
  ...
  },
  {
    ...
    header: 'Built for distributed teams',
  description: `Enable seamless collaboration across development, QA, and product teams with real-time sharing, comments, notifications, and role-based access controls.<ul style='margin-top:16px;text-align:left;'><li><b>Real-time Sharing:</b> Share test results and reports instantly with stakeholders</li><li><b>Comments & Reviews:</b> Collaborative test review process with inline comments</li><li><b>Role Management:</b> Granular permissions for different team roles and responsibilities</li><li><b>Activity Feeds:</b> Stay updated with real-time notifications and activity streams</li></ul>`,
    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
        {cards.slice(0, 4).map((card, idx) => (
          <div
            key={idx}
            className="comprehensive_card_main position-relative bg-white/80 feature-gradient-bg"
            ...
            {openIndex !== idx ? (
              <div className="text-center" style={{ cursor: 'pointer' }}>
                ...
                <div>
                  ...
                  <p className="font-medium comprehensive_card_subHeader" style={{ color: '#dd246a', fontSize: 12, fontWeight: 500 }}>{card.header}</p>
                </div>
              ...
              <>
                ...
                <div className="text-center" style={{ marginTop: 32, color: '#fff' }}>
                  ...
                  <p className="font-medium comprehensive_card_subHeader" style={{ color: '#dd246a', fontSize: 12, fontWeight: 500, marginTop: 8 }}>{card.header}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {cards.slice(4).map((card, idx) => (
          <div
            key={idx + 4}
            className="comprehensive_card_main bg-white/80 feature-gradient-bg"
            style={cardStyle(openIndex === idx + 4)}
            aria-haspopup="dialog"
            aria-expanded={openIndex === idx + 4}
            aria-controls={`card-${idx + 4}`}
            data-state={openIndex === idx + 4 ? 'open' : 'closed'}
            onClick={() => openIndex === null && setOpenIndex(idx + 4)}
          >
            {openIndex !== idx + 4 ? (
